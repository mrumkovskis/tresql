...
trait Transformer { self: QueryBuilder =>
  ...
  def transform(expr: Expr, f: PartialFunction[Expr, Expr]): Expr = {
    ...
    cf = f.orElse[Expr, Expr] {
      ...
      case FunAsTableExpr(e, cd, ord) => FunAsTableExpr(cf(e), cd.map(cf(_).asInstanceOf[TableColDefsExpr]), ord)
      case TableColDefsExpr(tcd) => TableColDefsExpr(tcd.map(cf(_).asInstanceOf[TableColDefExpr]))
