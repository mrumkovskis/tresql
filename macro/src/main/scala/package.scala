...
package object tresql extends CoreTypes {
  ...
  /**
    ...
    class Dept extends CompiledRow {
      ...
      object Emp extends RowConverter[Dept] {
  ...
  */
  ...
  object Macros {
    ...
    def impl(c: Context)(params: c.Expr[Any]*)(resources: c.Expr[Resources]): c.Expr[Result] = {
      ...
      def resultClassTree(exp: Exp): Ctx = {
        lazy val generator: PartialFunction[(Ctx, Exp), Ctx] = extractorAndTraverser {
          case (ctx, sd: SelectDef) =>
            ...
            val colsCtx = sd.cols
              .foldLeft(ColsCtx(Nil, 0, ctx.convRegister)) { case (colCt, c) =>
                val ct = generator(Ctx(null, null, ctx.depth, colCt.colIdx, colCt.convRegister) -> c)
                ColsCtx(ct.tree :: colCt.colTrees, colCt.colIdx + 1, ct.convRegister)
              }
            val (fieldDefs, fieldConvs) = colsCtx.colTrees
              .map (l => l.dropRight(1) -> l.last) //last value of every list goes to converter
              .foldLeft((List[c.Tree](), List[c.Tree]())) {
                case (r, x) => (x._1 ++ r._1, x._2 :: r._2)
              }
            ...
            """
            val converterDef = q"""
              object ${TermName(className)} extends org.tresql.CoreTypes.RowConverter[$typeName] {
            ...
            """
            val converterRegister = q"((${ctx.depth + 1} , ${ctx.colIdx}), $converterDef)" //depth for select starts with 1
            (Ctx(
              className,
              List(classDef, converterDef),
              ctx.depth,
              ctx.colIdx,
              converterRegister :: colsCtx.convRegister
            ), false)
          case (ctx, ColDef(name, ChildDef(sd: SelectDef), typ)) =>
            val selDefCtx = generator(Ctx(null, null, ctx.depth + 1, ctx.colIdx, ctx.convRegister) -> sd)
            val fieldDef =
              q"""var ${TermName(name)}: org.tresql.CompiledResult[${TypeName(selDefCtx.className)}] = _"""
            val fieldConv = q"""obj.${TermName(name)} =
              row.typed[org.tresql.CompiledResult[${TypeName(selDefCtx.className)}]](${ctx.colIdx})"""
            (Ctx(
              null,
              (fieldDef :: selDefCtx.tree) :+ fieldConv,
              ctx.depth,
              ctx.colIdx,
              selDefCtx.convRegister
            ), false)
          ...
          case (ctx, ColDef(name, _, typ)) =>
            ...
            val l = List(q"var ${TermName(name)}: ${TypeName(typ.toString)} = _",
             q"obj.${TermName(name)} = row.typed[${TypeName(typ.toString)}](${ctx.colIdx})")
            (ctx.copy(tree = l), false)
        }
        generator((Ctx(null, Nil, 0, 0, Nil), exp))
      }
      val macroSettings = settings(c.settings)
      val verbose = macroSettings.contains("verbose")
      val nocompile = macroSettings.contains("nocompile")
      ...
      if (!nocompile) Env.metaData = metadata(macroSettings)
      log(s"Compiling: $tresqlString")
      val compiledExp = if (nocompile) null else compile(tresqlString)
      val resultClassCtx = if (nocompile) null else resultClassTree(compiledExp)
